let quotes = {
    Life: [
        { quote: "Life is what happens when you're busy making other plans.", author: "John Lennon" },
        { quote: "Get busy living or get busy dying.", author: "Stephen King" },
        { quote: "You only live once, but if you do it right, once is enough.", author: "Mae West" },
        { quote: "Life is really simple, but we insist on making it complicated.", author: "Confucius" },
        { quote: "In the end, it’s not the years in your life that count. It’s the life in your years.", author: "Abraham Lincoln" },
    ],
    Death: [
        { quote: "Death is not the opposite of life, but a part of it.", author: "Haruki Murakami" },
        { quote: "To the well-organized mind, death is but the next great adventure.", author: "J.K. Rowling" },
        { quote: "The life of the dead is placed in the memory of the living.", author: "Marcus Tullius Cicero" },
        { quote: "Our dead are never dead to us, until we have forgotten them.", author: "George Eliot" },
        { quote: "Death may be the greatest of all human blessings.", author: "Socrates" },
    ],
    Happiness: [
        { quote: "Happiness is not something ready-made. It comes from your own actions.", author: "Dalai Lama" },
        { quote: "For every minute you are angry, you lose sixty seconds of happiness.", author: "Ralph Waldo Emerson" },
        { quote: "Happiness depends upon ourselves.", author: "Aristotle" },
        { quote: "Happiness is not by chance, but by choice.", author: "Jim Rohn" },
        { quote: "The purpose of our lives is to be happy.", author: "Dalai Lama" },
    ],
    Sadness: [
        { quote: "Tears come from the heart and not from the brain.", author: "Leonardo da Vinci" },
        { quote: "Sadness flies away on the wings of time.", author: "Jean de La Fontaine" },
        { quote: "The word 'happiness' would lose its meaning if it were not balanced by sadness.", author: "Carl Jung" },
        { quote: "Every human walks around with a certain kind of sadness.", author: "Henri Nouwen" },
        { quote: "Even a happy life cannot be without a measure of darkness.", author: "Carl Jung" },
    ],
    Truth: [
        { quote: "In a time of deceit, telling the truth is a revolutionary act.", author: "George Orwell" },
        { quote: "If you tell the truth, you don't have to remember anything.", author: "Mark Twain" },
        { quote: "Three things cannot be long hidden: the sun, the moon, and the truth.", author: "Buddha" },
        { quote: "The truth will set you free, but first it will make you miserable.", author: "James A. Garfield" },
        { quote: "Honesty is the first chapter in the book of wisdom.", author: "Thomas Jefferson" },
    ],
    Success: [
        { quote: "Success is not final, failure is not fatal: It is the courage to continue that counts.", author: "Winston Churchill" },
        { quote: "Success usually comes to those who are too busy to be looking for it.", author: "Henry David Thoreau" },
        { quote: "The way to get started is to quit talking and begin doing.", author: "Walt Disney" },
        { quote: "Don’t be afraid to give up the good to go for the great.", author: "John D. Rockefeller" },
        { quote: "Success is walking from failure to failure with no loss of enthusiasm.", author: "Winston Churchill" },
    ],
    Motivation: [
        { quote: "The only way to do great work is to love what you do.", author: "Steve Jobs" },
        { quote: "The future belongs to those who believe in the beauty of their dreams.", author: "Eleanor Roosevelt" },
        { quote: "Don’t watch the clock; do what it does. Keep going.", author: "Sam Levenson" },
        { quote: "Believe you can and you’re halfway there.", author: "Theodore Roosevelt" },
        { quote: "Start where you are. Use what you have. Do what you can.", author: "Arthur Ashe" },
    ]
};

let quote = document.querySelector(".quote");
let author = document.querySelector(".author");

function getRandomQuote(category) {
    let selectedQuotes = quotes[category];
    return selectedQuotes[Math.floor(Math.random() * selectedQuotes.length)];
}

function showRandomQuote(category) {
    const categoryQuotes = quotes[category];
    const randomQuote = categoryQuotes[Math.floor(Math.random() * categoryQuotes.length)];

    document.querySelector(".quote h1").textContent = `"${randomQuote.quote}"`;
    document.querySelector(".quote p").textContent = `~${randomQuote.author}~`;
}

// Set up event listeners for each category button
document.querySelectorAll(".category input").forEach(button => {
    button.addEventListener("click", () => showRandomQuote(button.value));
});

